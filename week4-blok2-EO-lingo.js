...
for (var i = 2; i <= 5; i++) {
	gebid('v1'+i).innerHTML = lingoword[0];
	gebid('v1').innerHTML = lingoword[0];
}
...
function gekko () {
	...
	for (var i = 0; i < 5; i++) {
			...
			gebid('v'+(i+1)).innerHTML = letterinput[i];
			gebid('v'+(i+1)).style.borderRadius = "0";
			gebid('v'+(i+1)).style.backgroundColor = "white"

		if (lingoword[i] == letterinput[i]) {
			gebid('v'+(i+1)).style.backgroundColor = "green"
			winaudio.play();
...
for (var i = 0; i < 5; i++) {
	for (var j = 0; j < 5; j++) {
		if (i != j && lingoword[i] == letterinput[j]) {
			if(gebid('v'+(j+1)).style.backgroundColor != 'green'){
				gebid('v'+(j+1)).style.backgroundColor = "yellow"
				gebid('v'+(j+1)).style.borderRadius = "50px";
...
function blokjes(parentElement, kolom, rij) {
	for ( var k = 1; k <=  kolom ; k++) {
		...
		for (var p = 1; p <= rij ; p++) {
		var cid = document.createElement ("id")	
		parentElement.appendChild(cid)	
		cid.setAttribute('class', 'vierkant')
		cid.setAttribute('id','r' + rij +  'k' + kolom ) 
